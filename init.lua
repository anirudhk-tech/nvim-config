-- ========================
-- brew install fd ripgrep fzf node python@3.12 go rustup-init llvm sqlite libpq mariadb-client wget unzip git-delta
-- ========================
...
require("lazy").setup({
  ...
  -- Snacks.nvim (The High Performance Picker)
  {
    "folke/snacks.nvim",
    priority = 1000,
    lazy = false,
    opts = {
      picker = {
        enabled = true,
        -- High-perf settings for big repos
        ui_select = true,
        sources = {
          files = {
            hidden = true,   -- Show hidden files (.env, etc)
            ignored = false, -- Respect .gitignore but show if you need to
          },
        },
      },
      -- Optional: adds nice UI for other things
...
map("n", "<leader><leader>", function() Snacks.picker.smart() end, { desc = "Smart Find" })
